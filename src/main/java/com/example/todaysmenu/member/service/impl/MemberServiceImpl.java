...
public class MemberServiceImpl implements MemberService {
    ...
    public String  register(MemberDTO memberDTO
        ...
        if(!idMatcher.matches() || loginId.length() < 6) {
            comModal.redirect("join.do",rttr,"아이디 유효성 검사","아이디는 영소문자,숫자로 구성된 6글자 이상으로 조합하시오.",danger);
        }

        if(!emailMatcher.matches()) {
            comModal.redirect("join.do",rttr,"이메일 유효성 검사","이메일 형식에 맞게 입력해주세요.",danger);
        }
        ...
        try {
            ...
            if (result == 1) {
                ...
                comModal.redirect("join.do",rttr,msg + " 성공메시지",msg+ "에 성공했습니다.",success);
            }
        } catch (Exception e) {
            log.info("result{}",result);
            if(result == 0) {
                comModal.redirect("join.do",rttr,"실패 메시지","이미 존재하는 회원입니다.",danger);
            }
            // 무결성 제약 조건 위배 예외 처리
    ...
    public String login(MemberDTO memberDTO, RedirectAttributes rttr, HttpSession session) {
        if (memberDTO.getTmt_login_id() == null || memberDTO.getTmt_login_id().equals("")||
            ...
            comModal.redirect("loginForm.do",rttr,"실패 메시지","모든 내용을 입력해주세요",danger);
        }
        MemberDTO mvo = memberRepository.login(memberDTO);
        if(mvo != null) { //로그인 성공
            session.setAttribute("memberDTO",mvo); // ${empty mvo} 헤더에서 체크하고 있음
            comModal.redirect("",rttr,"성공 메세지","로그인에 성공했습니다.",success);
        }else{ //로그인 실패
            comModal.redirect("loginForm.do",rttr,"로그인에 실패했습니다","로그인에 실패했습니다.",danger);
